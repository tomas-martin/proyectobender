...
class _AgregarPropiedadVistaState extends State<AgregarPropiedadVista> {
  ...
  String _tipoSeleccionado = 'Departamento';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.propiedad == null ? 'Nueva Propiedad' : 'Editar Propiedad'),
        backgroundColor: Theme.of(context).colorScheme.surface,
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          padding: const EdgeInsets.fromLTRB(20, 20, 20, 80), // bottom padding para evitar overflow
          child: Form(
            key: _formKey,
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: _guardando ? null : () => setState(() => _imagenSeleccionada = null),
                ...
                const Text('O pega una URL de imagen', style: TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(height: 8),
                TextFormField(
                ...
                const Text('Alquiler Mensual', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                TextFormField(
                  ...
                  validator: (value) {
                    if (value == null || value.trim().isEmpty) return 'El alquiler es obligatorio';
                    if (double.tryParse(value.trim()) == null) return 'Ingresa un número válido';
                    return null;
                  },
                ),
                const SizedBox(height: 24),

                const Text('Tipo de Propiedad', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                  ...
                  child: Column(
                    children: [
                      RadioListTile<String>(
                        ...
                        title: Row(
                          children: const [
                            ...
                            SizedBox(width: 8),
                            Text('Casa', style: TextStyle(color: Colors.white, fontSize: 14)),
                          ],
                        ),
                        value: 'Casa',
                      ...
                      RadioListTile<String>(
                        ...
                        title: Row(
                          children: const [
                            Icon(Icons.apartment, color: Colors.orange, size: 18),
                            SizedBox(width: 8),
                            Text('Departamento', style: TextStyle(color: Colors.white, fontSize: 14)),
                          ],
                        ),
                        value: 'Departamento',
                        groupValue: _tipoSeleccionado,
                ...
                const Text('Propietario', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                ...
                propietariosVM.propietarios.isEmpty
                  ...
                  hint: const Text('Selecciona un propietario', style: TextStyle(color: Colors.white54)),
                      ...
                      child: Row(
                        children: [
                          CircleAvatar(
                            ...
                            child: Text(prop.nombre.isNotEmpty ? prop.nombre[0].toUpperCase() : '?',
                  ...
                  onChanged: (value) => setState(() => _propietarioIdSeleccionado = value),
                  validator: (value) {
                    if (value == null) return 'Selecciona un propietario';
                    return null;
                  },
                ),
                ...
                const Text('Estado de la Propiedad', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                const Text('Define si la propiedad está disponible o alquilada', style: TextStyle(color: Colors.white54, fontSize: 12, fontStyle: FontStyle.italic)),
                      ...
                      RadioListTile<String>(
                        ...
                        title: Row(
                          children: const [
                            ...
                            SizedBox(width: 8),
                            Text('Disponible', style: TextStyle(color: Colors.white, fontSize: 14)),
                          ],
                        ),
                        subtitle: const Text('Lista para alquilar', style: TextStyle(color: Colors.white54, fontSize: 11)),
                        value: 'disponible',
                      ...
                      RadioListTile<String>(
                        ...
                        title: Row(
                          children: const [
                            Icon(Icons.home, color: Colors.blue, size: 18),
                            SizedBox(width: 8),
                            Text('Alquilada', style: TextStyle(color: Colors.white, fontSize: 14)),
                          ],
                        ),
                        subtitle: const Text('Actualmente alquilada', style: TextStyle(color: Colors.white54, fontSize: 11)),
                        value: 'alquilada',
                        groupValue: _estadoSeleccionado,
                        activeColor: Colors.amber,
                        onChanged: (value) => setState(() => _estadoSeleccionado = value!),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: (_guardando || _subiendoImagen || propietariosVM.propietarios.isEmpty) ? null : _guardarPropiedad,
                    icon: (_guardando || _subiendoImagen) ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.black)) : const Icon(Icons.save),
                    label: Text(_subiendoImagen ? 'Subiendo imagen...' : _guardando ? 'Guardando...' : 'Guardar Propiedad'),
  ...
  Widget _buildCampoTexto(String label, TextEditingController controller, String hint, IconData icono) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        TextFormField(
          ...
          validator: (value) => value == null || value.trim().isEmpty ? '$label es obligatorio' : null,
        ),
      ],
    );
  }
